...
<head>
    ...
    <link rel="stylesheet" href="../css/compilerStyle.css">
...
<main>
  <div class="condition">
    <?php echo $task['descript']; ?>
  </div>
<p class="text-slate-200 text-3xl my-4 font-extrabold mx-2 pt-8 start">Поехали!</p>
...
<form id="feedback" method="post" onsubmit="return false">
    <div class="h-3/4 flex flex-row">
        ...
        <div id="compiler_window" class="grid w-2/3 border-dashed border-2 border-slate-500 mx-2">
            <!-- our code editor, where codemirror renders it's editor -->
            <textarea id="code" name="code" class="h-full"></textarea>
        </div>

        <div id="result_window" class="grid w-1/3 border-dashed border-2 border-slate-500 mx-2">
    ...
    <div class="buttons">
      <button onclick="evaluatePython()" type="button" class="mx-2 my-4 h-12 px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm bg-green-700 hover:bg-green-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-700 text-slate-300 run">Запустить</button>
      <!-- clean the output section -->
      <button onclick="clearHistory()" type="button" class="mx-2 my-4 h-12 px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm bg-red-700 hover:bg-red-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-700 text-slate-300 clear">Очистить</button>
    </div>
    <script src="/practicode.ru/static/js/main.js"></script>
</form>

<a id="back" href="../task.php"><button class="send" type="submit" name="button-send-task"><i class="fa-solid fa-house"></i>Назад</button></a>
<p class="base">Ответ необходимо расположить в поле ниже в виде функции.</p>


<form name="result" method="post">
    <div id="compiler_window" class="grid w-2/3 border-dashed border-2 border-slate-500 mx-2">
        <!-- our code editor, where codemirror renders it's editor -->
        <textarea name="result_code" id="input" class="h-full"><?php echo $msg;?></textarea>
    </div>

    <button id="go" name="button-submit-result" type="submit" class="mx-2 my-4 h-12 px-6 py-3 border border-transparent text-base font-medium rounded-md shadow-sm bg-green-700 hover:bg-green-900 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-green-700 text-slate-300 run">Отправить</button>
